...
- [x] Supabase の Auth リダイレクト URL 追加（ダッシュボードで設定済み。本番ログイン成功で確認）
- [x] Worker を本番デプロイ（secrets 設定済み、CORS を `localhost:3000 + kataribe-nine.vercel.app` に）
- [x] 本番 URL でゲーム実機の通常フロー（キャプチャ → 読んで → 音声 → あらすじ)を一通り確認

### 動作確認

- [x] レート上限に達すると 429 が返り、フロントに残数と案内が出る（実装・コード検証済み。実表示は実運用の上限到達時に確認。`wrangler kv key put` でカウンタを直接書けばいつでも再現可能）
- [x] 主要エラーパスすべてで「無言の失敗」がない（監査で列挙した全パスを修正。個別のエラー表示はコード検証、通常フローはブラウザ確認済み）
- [x] 本番環境で通常フロー（REQUIREMENTS.md §1.3)が完走する
